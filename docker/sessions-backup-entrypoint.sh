...
if [ -n "${BACKUP_ENCRYPTION_KEY}" ]; then
  ...
  echo "[sessions-backup] FATAL: BACKUP_ENCRYPTION_KEY não foi definida." >&2
  echo "[sessions-backup] Backups de sessão contêm tokens sensíveis e DEVEM ser cifrados." >&2
  echo "[sessions-backup] Defina BACKUP_ENCRYPTION_KEY no ambiente Coolify e reinicie o serviço." >&2
  exit 1
fi
