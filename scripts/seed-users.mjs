...
const SHOW_SEED_CREDENTIALS = String(
  process.env.SEED_SHOW_CREDENTIALS || (IS_PRODUCTION ? "false" : "true"),
...
try {
  ...
  if (SHOW_SEED_CREDENTIALS) {
    console.log("[seed] Login credentials for this environment:");
    for (const user of USERS) {
      console.log(`[seed] LOGIN ${user.role.padEnd(5)} ${user.email}  password=${user.password}`);
    }
  }
} catch (err) {
