...
const AMAZON_HEALTH_URL = "http://127.0.0.1:3117/health";
...
function readOpsPortOverrides() {
  try {
    if (!existsSync(OPS_PORTS_CONFIG_PATH)) return {};
    const raw = readFileSync(OPS_PORTS_CONFIG_PATH, "utf8");
    const parsed = JSON.parse(raw || "{}");
    if (!parsed || typeof parsed !== "object") return {};

    const out = {};
    for (const [key, value] of Object.entries(parsed)) {
      if (isValidPort(value)) out[key] = Number(value);
    }
    return out;
  } catch {
    return {};
...
function saveOpsPortOverrides(overrides) {
  try {
    mkdirSync(OPS_CONFIG_DIR, { recursive: true });
    writeFileSync(OPS_PORTS_CONFIG_PATH, `${JSON.stringify(overrides, null, 2)}\n`, "utf8");
  } catch (error) {
    console.warn(`[preview:ready] Não foi possível salvar ${OPS_PORTS_CONFIG_PATH}: ${errorMessage(error)}`);
  }
}

function pickPreferredPort(overrides, key, fallbackPort) {
  const override = overrides?.[key];
  return isValidPort(override) ? Number(override) : fallbackPort;
}
...
async function findFreePort(startPort, maxAttempts = 50, excludedPorts = new Set()) {
...
async function ensureServiceOnline(serviceConfig) {
  ...
  if (!portFree) {
    servicePort = await findFreePort(defaultPort + 1, 200, STACK_RESERVED_PORTS);
...
async function main() {
  ...
  const persistedPortOverrides = readOpsPortOverrides();
  ...
  const supportServiceConfigs = [
    {
      ...
      healthPath: "/health",
      healthHeaders: { "x-ops-token": PREVIEW_OPS_CONTROL_TOKEN },
      defaultPort: 3115,
      buildScript: null,
    ...
    },
    {
      ...
      defaultPort: pickPreferredPort(persistedPortOverrides, "whatsapp", 3111),
    ...
    },
    {
      ...
      healthPath: "/health",
      healthHeaders: { "x-webhook-secret": PREVIEW_WEBHOOK_SECRET },
      defaultPort: pickPreferredPort(persistedPortOverrides, "telegram", 3112),
    ...
    },
    {
      ...
      healthPath: "/health",
      healthHeaders: { "x-webhook-secret": PREVIEW_WEBHOOK_SECRET },
      defaultPort: pickPreferredPort(persistedPortOverrides, "shopee", 3113),
    ...
    },
    {
      ...
      healthHeaders: { "x-webhook-secret": PREVIEW_WEBHOOK_SECRET },
      defaultPort: pickPreferredPort(persistedPortOverrides, "meli", 3114),
    ...
    },
    {
      serviceId: "amazon",
      name: "Amazon",
      healthUrl: AMAZON_HEALTH_URL,
      healthPath: "/health",
      healthHeaders: { "x-webhook-secret": PREVIEW_WEBHOOK_SECRET },
      defaultPort: pickPreferredPort(persistedPortOverrides, "amazon", 3117),
      buildScript: "svc:amazon:build",
      distPath: "services/amazon-affiliate/dist/server.js",
      envKey: "VITE_AMAZON_MICROSERVICE_URL",
      runtimeEnv: () => ({
        HOST: "0.0.0.0",
      }),
    },
  ...
  const failures = [];
  const resolvedServicePorts = { ...persistedPortOverrides };

  function resolvePortFromEndpoint(endpoint) {
    try {
  ...
  for (let i = 0; i < supportResults.length; i++) {
    ...
    if (result.status === "fulfilled") {
      ...
      if (config.serviceId && config.serviceId !== "ops") {
        const resolvedPort = resolvePortFromEndpoint(result.value.endpoint);
        if (resolvedPort) {
          resolvedServicePorts[config.serviceId] = resolvedPort;
        }
      }
    } else {
  ...
  saveOpsPortOverrides(resolvedServicePorts);
  ...
  const apiConfig = {
    ...
    runtimeEnv: () => ({
      ...
      AMAZON_MICROSERVICE_URL: runtimeEndpoints.VITE_AMAZON_MICROSERVICE_URL,
