...
export async function loadPlanMappings(): Promise<KiwifyPlanMapping[]> {
  ...
  // Webhook lookup uses findPlanByKiwifyProduct() which correctly filters by is_active.
  const rows = await query("SELECT * FROM kiwify_plan_mappings ORDER BY plan_id");
  return rows as KiwifyPlanMapping[];
}

export async function findPlanByKiwifyProduct(kiwifyProductId: string): Promise<KiwifyPlanMapping | null> {
  const row = await queryOne(
    "SELECT * FROM kiwify_plan_mappings WHERE kiwify_product_id = $1 AND is_active = TRUE",
    [kiwifyProductId]
  );
  return row as KiwifyPlanMapping | null;
}
