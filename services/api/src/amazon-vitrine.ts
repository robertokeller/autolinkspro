...
const AMAZON_VITRINE_SYNC_INTERVAL_MS = Number.parseInt(
  process.env.AMAZON_VITRINE_SYNC_INTERVAL_MS || "21600000",
  10
); // 6 horas
...
export const AMAZON_VITRINE_TABS: AmazonVitrineTabConfig[] = [
  {
    // Amazon search sorted by highest discount % — server-side rendered, stable structure
    key: "destaques",
    label: "Destaques",
    sourceUrl: "https://www.amazon.com.br/events/ofertasmensais",
  },
  {
    // Computers/notebooks sorted by discount
    key: "top_performance",
    label: "Top Performance",
    sourceUrl: "https://www.amazon.com.br/events/ofertasmensais?bubble-id=discounts-collection-for-you",
  },
  {
    // Electronics sorted by sales rank
    key: "mais_vendidos",
    label: "Mais Vendidos",
    sourceUrl: "https://www.amazon.com.br/events/ofertasmensais?bubble-id=deals-collection-pix",
  },
  {
    // Smartphones sorted by highest discount
    key: "ofertas_quentes",
    label: "Ofertas Quentes",
    sourceUrl: "https://www.amazon.com.br/events/ofertasmensais?bubble-id=deals-todasofertas",
  },
  {
    // Products sorted by review count — best rated products
    key: "melhor_avaliados",
    label: "Melhor Avaliados",
    sourceUrl: "https://www.amazon.com.br/events/ofertasmensais?bubble-id=deals-maisamadas",
  },
];
  ...
  try {
    if (!input.force && onlyIfStale) {
      ...
      if (!stale) {
        ...
        return {
          ...
          message: "Sync ignorado porque a vitrine ainda está dentro da janela de 6h.",
        };
      }
    }

    const extractedByTab = new Map<AmazonVitrineTabConfig, ExtractedProduct[]>();
    const extractionWarnings: string[] = [];
    let fetchedCards = 0;

    for (const tab of AMAZON_VITRINE_TABS) {
      try {
        console.log(`[Amazon Vitrine] Extracting from ${tab.key}...`);
        const html = await fetchHtml(tab.sourceUrl);
        const extracted = extractProductsFromHtml(html, tab.key, tab.sourceUrl);

        if (extracted.length === 0) {
          extractionWarnings.push(`[${tab.key}] sem produtos válidos na página`);
          continue;
        }

        const enriched = extracted;
        console.log(`[Amazon Vitrine] ${tab.key}: ${enriched.length} products extracted.`);

        if (enriched.length === 0) {
          extractionWarnings.push(`[${tab.key}] nenhum produto com imagem`);
          continue;
        }

        fetchedCards += enriched.length;
        const shuffled = shuffleArray(enriched);
        extractedByTab.set(tab, shuffled);
      } catch (tabError) {
        extractionWarnings.push(`[${tab.key}] ${normalizeErrorMessage(tabError)}`);
      }
    }
    ...
    return {
      ...
      message: extractionWarnings.length > 0
        ...
        : "Sync da vitrine Amazon concluido com sucesso.",
    };
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
