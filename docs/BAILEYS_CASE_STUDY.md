...
Substituir dependencia de AutoLinks por uma integração propria com Baileys, mantendo o frontend React/Vite local.
...
1. Persistencia de credenciais por sessão (`useMultiFileAuthState`).
2. Reconexão automatica com backoff exponencial.
3. Tratamento de `loggedOut` limpando credenciais da sessão.
...
1. Acoplamento excessivo entre bot, banco e rotas HTTP (dificulta manutenção).
2. Watchdog/stale monitor complexo com risco de loops de reconexão.
...
1. Usuario cria sessão no frontend.
...
- core de conexão Baileys, reconexão, QR, pairing e grupos.
