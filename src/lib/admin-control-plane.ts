import { getPlanFeatureList, plans as staticPlans, type Plan, type PlanLimits, type PlanPeriodConfig, type BillingPeriodType, type CheckoutProvider } from "@/lib/plans";
...
function normalizeCheckoutProvider(value: unknown, fallback: CheckoutProvider = "kiwify"): CheckoutProvider {
  if (value === "kiwify") return value;
  return fallback;
}
      ...
      .map((plan, index) => {
        ...
        const normalizedPeriods: PlanPeriodConfig[] = (() => {
          ...
          if (Array.isArray(stored) && stored.length > 0) {
            const planProvider = normalizeCheckoutProvider(
              (plan as { checkoutProvider?: unknown }).checkoutProvider,
              normalizeCheckoutProvider((basePlan as { checkoutProvider?: unknown }).checkoutProvider, "kiwify"),
            );
            return stored
              ...
              .map((p) => ({
                ...
                kiwifyCheckoutUrl: typeof (p as PlanPeriodConfig).kiwifyCheckoutUrl === "string"
                  ...
                  : undefined,
                checkoutProvider: normalizeCheckoutProvider(
                  (p as PlanPeriodConfig).checkoutProvider,
                  planProvider,
                ),
                isActive: Boolean((p as PlanPeriodConfig).isActive ?? true),
              }));
          }
          return (basePlan.periods ? [...basePlan.periods] : []).map((period) => ({
            ...period,
            checkoutProvider: normalizeCheckoutProvider(
              period.checkoutProvider,
              normalizeCheckoutProvider((basePlan as { checkoutProvider?: unknown }).checkoutProvider, "kiwify"),
            ),
          }));
        })();
        ...
        return {
          ...
          kiwifyProductId: typeof (plan as { kiwifyProductId?: unknown }).kiwifyProductId === "string"
            ? (plan as { kiwifyProductId?: string }).kiwifyProductId
            : (typeof (basePlan as { kiwifyProductId?: unknown }).kiwifyProductId === "string"
              ? (basePlan as { kiwifyProductId?: string }).kiwifyProductId
              : undefined),
          kiwifyCheckoutUrl: typeof (plan as { kiwifyCheckoutUrl?: unknown }).kiwifyCheckoutUrl === "string"
            ? (plan as { kiwifyCheckoutUrl?: string }).kiwifyCheckoutUrl
            : (typeof (basePlan as { kiwifyCheckoutUrl?: unknown }).kiwifyCheckoutUrl === "string"
              ? (basePlan as { kiwifyCheckoutUrl?: string }).kiwifyCheckoutUrl
              : undefined),
          baseLimits,
