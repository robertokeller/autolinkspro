              ...
              <div className="space-y-5 lg:col-span-6">
                <div className="rounded-xl border p-4 space-y-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <input ref={fileInputRef} type="file" accept="image/png,image/jpeg,image/webp" className="hidden" onChange={(e) => { const f = e.target.files?.[0]; if (f) handleLogoUpload(f); e.target.value = ""; }} />
                    {form.logoUrl ? (
                    ...
                    {form.texts.testimonials.length === 0 ? (
                      ...
                      <div className="grid gap-3 min-[600px]:grid-cols-3">
                        {form.texts.testimonials.map((t, idx) => (
                          <div key={idx} className="relative rounded-lg border bg-card p-3 space-y-3">
                            <Button type="button" variant="ghost" size="icon" className="absolute top-1 right-1 h-6 w-6 text-muted-foreground hover:text-destructive" onClick={() => {
                              const nt = [...form.texts.testimonials];
                              nt.splice(idx, 1);
                              setForm(p => ({ ...p, texts: { ...p.texts, testimonials: nt } }));
                            }}>
                              <Trash2 className="h-3 w-3" />
                            </Button>
                            <div className="space-y-1 mt-2">
                              <Label className="text-2xs font-medium text-muted-foreground">Nome</Label>
                              <Input className="h-8 text-xs" placeholder="Ana C." value={t.name} onChange={(e) => {
                                const nt = [...form.texts.testimonials];
                            ...
                            <div className="space-y-1">
                              <Label className="text-2xs font-medium text-muted-foreground">Texto</Label>
                              <Textarea className="h-16 min-h-[4rem] text-xs resize-none" placeholder="Economizei muito!" value={t.text} onChange={(e) => {
                                const nt = [...form.texts.testimonials];
