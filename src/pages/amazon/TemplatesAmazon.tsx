...
import { AMAZON_TEMPLATE_MODULE } from "@/lib/marketplace-template-modules";
...
const DEFAULT_TEMPLATE_CONTENT = AMAZON_TEMPLATE_MODULE.defaultTemplateContent;
const PLACEHOLDER_LEGEND = AMAZON_TEMPLATE_MODULE.placeholderLegend;
const PREVIEW_SAMPLE = AMAZON_TEMPLATE_MODULE.previewSample;

type AmazonScheduleProductInput = {
  title?: string;
...
type GeneratedOffer = {
  ...
  product: AmazonScheduleProductInput;
};
...
function extractAmazonAsinFromUrl(rawUrl: string): string {
  try {
    const normalized = /^https?:\/\//i.test(rawUrl.trim()) ? rawUrl.trim() : `https://${rawUrl.trim()}`;
    const parsed = new URL(normalized);
...
export default function TemplatesAmazon() {
  const { user } = useAuth();
  const {
    templates,
    createTemplate,
  ...
  const { data: hasAmazonTagConfigured = false, isLoading: loadingAmazonTag } = useQuery({
    queryKey: ["amazon-affiliate-tag-status", user?.id],
  ...
  const [scheduleProduct, setScheduleProduct] = useState<AmazonScheduleProductInput | null>(null);
  ...
  const handleConvert = async () => {
    ...
    if (!link) {
      toast.error("Cole um link da Amazon primeiro.");
      return;
    }
    if (!isLikelyAmazonUrl(link)) {
      toast.error("Use um link válido da Amazon.");
      return;
    }
    if (!hasAmazonTagConfigured) {
      toast.error("Configure sua tag Amazon em Configurações Amazon para converter.");
      return;
    }
    ...
    try {
      const conversion = await convertMarketplaceLink({
        url: link,
        source: "templatesamazon-converter",
      });
      if (conversion.marketplace !== "amazon") {
        throw new Error("Link não reconhecido como Amazon.");
      }
      ...
      const message = AMAZON_TEMPLATE_MODULE.applyPlaceholders(template.content, placeholderData);
      ...
      setGeneratedOffer({
        ...
        affiliateLink,
        originalLink,
        conversionTimeMs: Number.isFinite(Number(conversion.conversionTimeMs)) ? Number(conversion.conversionTimeMs) : null,
  ...
  return (
    <div className="ds-page pb-[calc(var(--safe-area-bottom)+0.25rem)]">
      <PageHeader
        title="Templates Amazon"
        description="Monte templates e gere mensagens com conversão de link Amazon"
      >
        ...
        <Card className="glass xl:sticky xl:top-20">
          <CardHeader className="border-b pb-4">
            <CardTitle className="flex items-center gap-2 text-sm">
              ...
              Gerador de mensagem Amazon
            </CardTitle>
            <p className="text-xs text-muted-foreground">
              Cole um link da Amazon, converta para afiliado e gere a mensagem final com template.
            </p>
          </CardHeader>
          ...
          <CardContent className="space-y-4 pt-4">
            ...
            <div className="space-y-3">
              <div className="space-y-1.5">
                ...
                <Input
                  placeholder="Cole o link do produto Amazon"
                  value={converterLink}
              ...
              <div className="grid gap-2 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-end">
                ...
                <Button
                  ...
                  disabled={converting || !converterLink.trim() || !hasAmazonTagConfigured || templates.length === 0}
        ...
        <Card className="glass">
          ...
          <CardContent className="pt-4">
            {templates.length > 0 ? (
              ...
              <EmptyState
                icon={FileText}
                title="Nenhum template Amazon ainda"
      ...
      <Dialog open={showModal} onOpenChange={setShowModal}>
        <DialogContent className="flex flex-col gap-0 max-h-[92dvh] w-[min(calc(100vw-1rem),72rem)] max-w-none overflow-hidden p-0">
          <DialogHeader className="border-b px-6 py-4">
            <DialogTitle>{editing ? "Editar template Amazon" : "Novo template Amazon"}</DialogTitle>
          </DialogHeader>
          ...
          <div className="grid flex-1 min-h-0 overflow-hidden md:grid-cols-2">
            <div className="h-full space-y-4 overflow-y-auto px-6 py-5">
              <div className="space-y-2">
                <Label>Nome</Label>
                <Input
                  placeholder="Ex: Oferta padrao Amazon"
                  value={form.name}
              ...
              <div className="space-y-2">
                <Label className="text-xs text-muted-foreground">
                  Placeholders Amazon - clique para inserir no cursor
                </Label>
            ...
            <div className="flex h-full flex-col space-y-3 overflow-hidden border-t bg-muted/20 px-6 py-5 md:border-l md:border-t-0">
              <div className="shrink-0">
                ...
                <p className="mt-0.5 text-xs text-muted-foreground">
                  Render com dados de exemplo Amazon.
                </p>
              </div>
              <pre
