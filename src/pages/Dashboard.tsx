  ...
  const healthCards: readonly HealthCardViewModel[] = [
    {
      ...
      details:
        waSessions.length === 0
          ? "Nenhuma sessao"
          : waServiceOnline
            ? `${waOnline}/${waSessions.length} sessoes conectadas`
            : waError || `${waOnline}/${waSessions.length} sessoes conectadas`,
      statusText: waSessions.length === 0 ? "Não configurado" : !waServiceOnline ? "Erro no serviço" : waOnline === waSessions.length ? "Online" : waOnline === 0 ? "Offline" : "Parcial",
      statusTone: waSessions.length === 0 ? "secondary" : !waServiceOnline ? "destructive" : waOnline === waSessions.length ? "success" : "warning",
      icon: WhatsAppIcon,
    ...
    },
    {
      ...
      details:
        tgSessions.length === 0
          ? "Nenhuma sessao"
          : tgServiceOnline
            ? `${tgOnline}/${tgSessions.length} sessoes conectadas`
            : tgError || `${tgOnline}/${tgSessions.length} sessoes conectadas`,
      statusText: tgSessions.length === 0 ? "Não configurado" : !tgServiceOnline ? "Erro no serviço" : tgOnline === tgSessions.length ? "Online" : tgOnline === 0 ? "Offline" : "Parcial",
      statusTone: tgSessions.length === 0 ? "secondary" : !tgServiceOnline ? "destructive" : tgOnline === tgSessions.length ? "success" : "warning",
      icon: TelegramIcon,
    ...
    },
    {
      ...
      statusTone: !shopeeConfigured ? "secondary" : shopeeServiceOnline ? "success" : "destructive",
      icon: ShoppingBag,
    ...
    },
    {
      ...
      statusTone: meliSessions.length === 0 ? "secondary" : !meliServiceOnline ? "destructive" : analytics.meliActive >= meliSessions.length && meliSessions.length > 0 ? "success" : "warning",
      icon: ShoppingCart,
      iconColorClass: "text-primary",
      iconBgClass: "bg-primary/10",
    },
