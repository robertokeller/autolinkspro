...
export default function SettingsPage() {
  ...
  const initialTab = searchParams.get("tab") === "plano" ? "plano" : "conta";
  const [activeTab, setActiveTab] = useState<"conta" | "plano">(initialTab);
  ...
  useEffect(() => {
    if (searchParams.get("showPlans") !== "1") return;
    setActiveTab("plano");
    setPlansModalOpen(true);
    const nextParams = new URLSearchParams(searchParams);
    nextParams.delete("showPlans");
  ...
  const resolveCheckoutPeriodForPlan = useCallback((
    targetPlanId: string,
    preferred?: "monthly" | "quarterly" | "semiannual" | "annual",
  ) => {
    const plan = adminControlPlane.plans.find((item) => item.id === targetPlanId);
    const preferredType = preferred ?? plan?.billingPeriod ?? "monthly";
    const periods = Array.isArray(plan?.periods) ? plan.periods : [];
    const preferredPeriod = periods.find((period) => period.type === preferredType && period.isActive !== false);
    if (preferredPeriod) return preferredType;
    const firstActive = periods.find((period) => period.isActive !== false);
    return firstActive?.type ?? preferredType;
  }, [adminControlPlane.plans]);

  const openCheckoutForPlan = useCallback(async (
    targetPlanId: string,
    preferredPeriod?: "monthly" | "quarterly" | "semiannual" | "annual",
    options?: { silent?: boolean },
  ) => {
    const periodType = resolveCheckoutPeriodForPlan(targetPlanId, preferredPeriod);
    try {
      const session = await invokeBackendRpc<{
        provider?: string;
        period_type?: string;
        checkout_url?: string;
      }>("account-plan", {
        body: {
          action: "create_kiwify_checkout",
    ...
    <div className="ds-page">
      <div className="mx-auto w-full max-w-4xl space-y-6">
        ...
        <Tabs
          value={activeTab}
          onValueChange={(value) => {
            const nextTab = value === "plano" ? "plano" : "conta";
            setActiveTab(nextTab);
            const nextParams = new URLSearchParams(searchParams);
            if (nextTab === "plano") {
              nextParams.set("tab", "plano");
            } else {
                ...
                {isPlanExpired && (
                  <div className="rounded-lg border border-destructive/40 bg-destructive/5 p-3">
                    ...
                    <Button
                      ...
                      onClick={() => {
                        void (async () => {
                          const opened = await openCheckoutForPlan(
                            currentPlan.id,
                            currentPlan.billingPeriod ?? "monthly",
                          );
                          if (!opened) setPlansModalOpen(true);
                        })();
                      }}
                    >
                      Renovar via Kiwify
                      ...
                      <div key={item.label} className="space-y-2 rounded-lg border p-3">
                        <div className="flex items-center justify-between gap-2">
                          <div className="flex min-w-0 items-center gap-2">
                            ...
                            <span className="truncate text-xs font-medium leading-none">{item.label}</span>
                          </div>
                        ...
                        <div className="flex items-center justify-between gap-2">
                          ...
                          {isCurrentPlan ? (
                            <div className="flex items-center gap-1">
                              ...
                              {isPlanExpired ? <Badge variant="destructive">Vencido</Badge> : null}
                            </div>
                          ) : null}
                        </div>
                        ...
                        <Button
                          ...
                          onClick={() => {
                            void (async () => {
                              ...
                              const opened = await openCheckoutForPlan(plan.id, plansBillingPeriod, { silent: true });
                              if (opened) return;
                              ...
                              await handleChoosePlan(plan.id);
                            })();
                          }}
                        >
                ...
                <div className="space-y-3 text-sm">
                  ...
                  <div className="rounded-md border bg-muted/40 p-3 space-y-1.5 text-xs">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium text-success">− R${prorationDialog?.unusedCredit.toFixed(2).replace(".", ",")}</span>
                    </div>
