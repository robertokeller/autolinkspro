...
type AutomationSnapshotRow = Pick<MarketplaceAutomationRow, "id" | "destination_group_ids" | "master_group_ids" | "config">;

function countAutomationDestinationSlots(row: Pick<MarketplaceAutomationRow, "destination_group_ids" | "master_group_ids">): number {
...
export function useMarketplaceAutomacoes(marketplace: MarketplaceAutomationKind) {
  ...
  const invalidateAutomationQueries = useCallback(() => {
    qc.invalidateQueries({ queryKey: ["marketplace_automations"] });
  }, [qc]);

  const loadAutomationSnapshots = useCallback(async (): Promise<{
    allAutomations: AutomationSnapshotRow[];
    marketplaceAutomations: AutomationSnapshotRow[];
  }> => {
    ...
    const marketplaceRows = filterMarketplaceAutomations(
      rows as unknown as MarketplaceAutomationRow[],
      marketplace,
    ) as unknown as AutomationSnapshotRow[];

    return {
      allAutomations: rows,
      marketplaceAutomations: marketplaceRows,
    };
  }, [marketplace]);
  ...
  const createMutation = useMutation({
    mutationFn: async (input: CreateMarketplaceAutomationInput) => {
      ...
      if (!isAdmin) {
        ...
        const { allAutomations, marketplaceAutomations } = await loadAutomationSnapshots();

        const maxAutomations = marketplace === "meli"
          ? limits.meliAutomations
          : limits.automations;
        ...
        if (maxGroupSlots !== -1) {
          const slotsUsed = allAutomations.reduce(
            (sum, item) => sum + countAutomationDestinationSlots(item),
            0,
          );
      ...
      if (!isAdmin && (input.destinationGroupIds !== undefined || input.masterGroupIds !== undefined)) {
        ...
        if (maxGroupSlots !== -1) {
          const slotsUsedByOthers = allAutomations
            ...
            .reduce((sum, item) => sum + countAutomationDestinationSlots(item), 0);
