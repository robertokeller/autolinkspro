...
import type {
  CreateMarketplaceAutomationInput,
...
const MARKETPLACE_COPY: Record<MarketplaceAutomationKind, MarketplaceCopy> = {
  meli: {
...
function flattenVitrineTabTree(nodes: VitrineTabNode[], parentLabel = ""): Array<{
  key: string;
  label: string;
  activeCount: number;
}> {
...
function automationToForm(row: MarketplaceAutomationRow, marketplace: MarketplaceAutomationKind): FormState {
  const keywordFilters = readAutomationKeywordFilters(row.config);
  const sourceConfig = readMarketplaceAutomationConfig(row.config, marketplace);
  return {
    name: row.name,
    ...
    sessionId: readAutomationSessionId(row.config, row.session_id),
...
export interface MarketplaceAutomationController {
  automations: MarketplaceAutomationRow[];
  isLoading: boolean;
  createAutomation: (input: CreateMarketplaceAutomationInput) => Promise<unknown>;
  updateAutomation: (input: Partial<CreateMarketplaceAutomationInput> & { id: string }) => Promise<unknown>;
  toggleAutomation: (id: string, isActive: boolean) => Promise<unknown>;
  deleteAutomation: (id: string) => Promise<unknown>;
  duplicateAutomation: (automation: MarketplaceAutomationRow) => Promise<void>;
  pauseAllAutomations: () => Promise<unknown>;
  resumeAllAutomations: () => Promise<unknown>;
  refreshAllAutomations: () => Promise<unknown>;
...
interface MarketplaceAutomacoesPageProps {
  marketplace: MarketplaceAutomationKind;
  automationController: MarketplaceAutomationController;
}

export function MarketplaceAutomacoesPage({
  marketplace,
  automationController,
}: MarketplaceAutomacoesPageProps) {
  const copy = MARKETPLACE_COPY[marketplace];
  const vitrineRpcName = marketplace === "meli" ? "meli-vitrine-list" : "amazon-vitrine-list";
  const vitrineCatalogKey = `${marketplace}-vitrine-tab-catalog`;
  ...
  const { data: vitrineCatalog } = useQuery({
    queryKey: [vitrineCatalogKey],
    queryFn: async () => await invokeBackendRpc<VitrineCatalog>(vitrineRpcName, {
      body: {
        tab: "destaques",
        page: 1,
        limit: 1,
  ...
  useEffect(() => {
    ...
    void backend.from("history_entries").insert({
      ...
      source: copy.activationSource,
      destination: "tab:activated",
      status: "info",
      details: {
        message: copy.activationMessage,
        step: "tab_activated",
        sourceRun: "smart-tab",
      },
      direction: "system",
  ...
  }, [copy.activationMessage, copy.activationSource, user]);
  ...
  const openEdit = (automation: MarketplaceAutomationRow) => {
  ...
  const handleSubmit = async () => {
    ...
    if (form.vitrineTabs.length === 0) {
      toast.error(copy.noVitrineError);
      return;
    }
    ...
    setSubmitting(true);
    try {
      const input: CreateMarketplaceAutomationInput = {
        name: form.name.trim(),
      ...
      {isLoading ? (
        ...
        <div className="space-y-4">
          {automations.map((automation) => {
            const sessionLabel = getSessionLabel(readAutomationSessionId(automation.config, automation.session_id));
