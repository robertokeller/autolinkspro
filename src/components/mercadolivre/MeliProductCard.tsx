...
import { Copy, ExternalLink, CalendarDays, ShoppingCart, Star, Store } from "lucide-react";
import { cn } from "@/lib/utils";

export interface MeliVitrineItem {
  id: string;
  tab: string;
  title: string;
  productUrl: string;
...
interface MeliProductCardProps {
  item: MeliVitrineItem;
  onConvert: (item: MeliVitrineItem) => void;
  onSchedule: (item: MeliVitrineItem) => void;
...
export function MeliProductCard({ 
  item, 
  onConvert, 
  onSchedule, 
  isConverting, 
  isScheduling 
}: MeliProductCardProps) {
        ...
        <div className="flex items-center justify-between gap-2">
          ...
          {item.rating && item.rating > 0 && (
            <span className="flex items-center gap-0.5 text-xs text-muted-foreground shrink-0">
              ...
              {reviewsCountLabel && <span className="ml-0.5 opacity-70">({reviewsCountLabel})</span>}
            </span>
          )}
        </div>
