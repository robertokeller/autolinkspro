...
- `docs/CLOUDFLARE_ROLLOUT_PLAN.md` (cutover sem downtime com validacao por servico)
...
- `npm run cloudflare:plan`: simula (dry-run) provisioning de DNS + SSL/TLS no Cloudflare
- `npm run cloudflare:apply`: aplica provisioning no Cloudflare (idempotente)
- `npm run cloudflare:verify`: valida zona/DNS/settings e checks HTTP basicos
- `npm run cloudflare:verify:strict`: valida com exigencia de zona ativa + headers de edge
